...
public class TechTest {
  ...
  private ArrayList<Tech> techPersonnel = new ArrayList<Tech>();

  @Test
  public void TestTechSubscribers() {
    ...
    if (producer != null && job != null) {
      ...
      for (int i = 0; i < roster.length; i++) {
        ...
        Status techStatus = techPersonnel.get(0).getJobSheet().getStatus();
        // Check Job Status is same as the Musicians job sheet
        Assert.assertTrue(status.equals(techStatus));
        // Check this is Submitted
        Assert.assertTrue(techStatus instanceof SubmittedStatus);
        ...
        techStatus = techPersonnel.get(0).getJobSheet().getStatus();
        Assert.assertTrue(status.equals(techStatus));
        // Check this is Approved
        Assert.assertTrue(techStatus instanceof ApprovedStatus);
      }
    }
  }
}
